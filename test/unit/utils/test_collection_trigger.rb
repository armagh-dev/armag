...
class TestCollectionTrigger < Test::Unit::TestCase
  ...
  def test_expired_run
    ...
    Armagh::TriggerDocument.expects(:ensure_one_exists).with(:state => Armagh::Documents::DocState::READY, :type => '__COLLECT__name', :pending_actions => @actions)
    @trigger.start
    sleep RUN_SLEEP
    @trigger.stop
  end
  ...
  def test_trigger_individual_collection_error
    Armagh::TriggerDocument.expects(:ensure_one_exists).raises(RuntimeError.new('nope'))
  ...
  def test_trigger_updated_workflow
    Armagh::TriggerDocument.expects(:ensure_one_exists).never
