...
class TestLauncher < Test::Unit::TestCase
  ...
  def setup
    ...
    @utility_config = mock('utility_config')
    lc = mock
    lc.stubs(:log_level).returns(Armagh::Logging::FATAL)
    @launcher_config.stubs(:launcher).returns(lc)
    Armagh::Launcher.expects(:find_or_create_configuration).returns(@launcher_config)
    Armagh::Agent.expects(:find_or_create_configuration).returns(@agent_config)
    Armagh::Utils::Archiver.expects(:find_or_create_configuration).returns(@archiver_config)
    Armagh::Authentication::Configuration.expects(:find_or_create_configuration).returns(@authentication_config)
    Armagh::Actions::UtilityAction.expects(:find_or_create_all_configurations).returns(@utility_config)
