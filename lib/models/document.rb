    ...
    class Document < Model
      ...
      def add_dev_error(action_name, details)
        ...
        if details.is_a? Exception
          ...
          @db_doc['dev_errors'][action_name] << {'message' => details.to_s, 'timestamp' => Time.now.utc}
        end
        update_pending_work
      end
      ...
      def add_ops_error(action_name, details)
        ...
        if details.is_a? Exception
          ...
          @db_doc['ops_errors'][action_name] << {'message' => details.to_s, 'timestamp' => Time.now.utc}
        end
        update_pending_work
      end
      ...
      def save(new: false, logger: nil)
        ...
        if save_collection
          if new || internal_id.nil?
            @db_doc['_id'] = self.class.db_create(@db_doc, save_collection)
          else
            if @pending_publish && @published_id
              ...
              self.class.db_replace({'_id': internal_id}, @db_doc, save_collection)
            end
          end
        end

        self.class.db_delete({'_id': internal_id}) if delete_orig
